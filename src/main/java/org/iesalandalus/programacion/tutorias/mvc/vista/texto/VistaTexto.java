...
public class VistaTexto implements IVista {
	...
	public void comenzar() {
		Consola.mostrarCabecera("Gestión de las Reservas de Tutorías del IES Al-Ándalus");
		int ordinalOpcion;
		do {
			Consola.mostrarMenu();
	...
	public void buscarAlumno() {
		...
		try {
			...
			String mensaje = (alumno != null) ? alumno.toString() : "No existe dicho profesor.";
	...
	public void insertarCita() {
		...
		try {
			...
			controlador.insertar(cita);
			System.out.println("Cita insertada correctamente.");
		} catch (OperationNotSupportedException | IllegalArgumentException | NullPointerException |DateTimeParseException e) {
			System.out.println(e.getMessage());
		}
	}
	...
	public void listarCitasSesion(){
		...
		List<Cita> citas = controlador.getCitas(Consola.leerSesion());
		if (!citas.isEmpty()) {
			for (Cita cita : citas) {
				if (cita != null) 
					System.out.println(cita);
	...
	public void listarCitasAlumno(){
		...
		List<Cita> citas = controlador.getCitas(Consola.leerAlumno());
		if (!citas.isEmpty()) {
			for (Cita cita : citas) {
				if (cita != null) 
					System.out.println(cita);
